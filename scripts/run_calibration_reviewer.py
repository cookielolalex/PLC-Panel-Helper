...
def build_result(
    ...
    comparison_path = require(input_review_dir / "comparison_metrics.json")
    grading_path = require(input_review_dir / "grading_result.json")
    effective_manifest_path = require(input_review_dir / "effective_sheet_revision_manifest.json")
    ...
    if generation_complete.get("project_id") != project_id or generation_complete.get("status") != "PASS":
        raise SystemExit("generation_complete is not a passing result for the selected project")
    if freeze.get("project_id") != project_id:
        ...
        findings.append(
            {
                ...
                "finding": f"Unpaired output types require follow-up: {', '.join(unpaired_outputs)}.",
                "evidence_ids": ["comparison_metrics.pairings"],
            }
        )

    result = {
        "review_id": f"{reviewer_label}_{run_id}",
        "run_id": run_id,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a fresh local calibration reviewer process.")
    parser.add_argument("--run-id", required=True)
    parser.add_argument("--project-id", required=True)
    parser.add_argument("--input-review-dir", type=Path)
    parser.add_argument("--output", type=Path)
    parser.add_argument("--reviewer-label", default="project_reviewer")
    parser.add_argument("--blind-note", default="blind to other reviewer results and portfolio statistics")
    args = parser.parse_args()

    root = Path.cwd()
    input_review_dir = args.input_review_dir or root / "evals" / "runs" / args.run_id / "review"
    output = args.output or input_review_dir / "project_reviewer_result.json"
    result = build_result(
        root=root,
        run_id=args.run_id,
        project_id=args.project_id,
        input_review_dir=input_review_dir,
        reviewer_label=args.reviewer_label,
        output=output,
        blind_note=args.blind_note,
    )
