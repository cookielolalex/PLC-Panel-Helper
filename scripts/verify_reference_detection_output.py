...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify minimized reference detection v3 outputs.")
    ...
    if not errors:
        ...
        errors.extend(validate_items(pages, ROOT / "schemas" / "reference_page_classification.schema.json", "page"))
        errors.extend(validate_items(docs, ROOT / "schemas" / "reference_document_classification.schema.json", "document"))
        errors.extend(validate(read_json(effective_path), read_json(ROOT / "schemas" / "effective_reference_set.schema.json")))
        errors.extend(validate(read_json(audit_path), read_json(ROOT / "schemas" / "reference_detection_audit.schema.json")))
